...
<template>
  <q-item multiline class="no-padding cursor-pointer"
          :class="{'claim-list-active': claimIdx === activeIndex, 'af-selectable' : isNotTouch}">
        ...
        <div v-if="claimRec.executor || claimRec.hasBuildTo">
          <div class="claim-row-c">
            ...
            <div class="col-sm-6 col-xs-12" style="margin-bottom: 5px" v-if="claimRec.executor">
              <div v-if="claimRec.executor">Виконавець&nbsp;<span class="token bg-teal-14">{{claimRec.executor}}</span>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    regDate () {
      return this.$routines.formatDate(this.$props['claimRec'].regDate)
    },
    isClosed () {
    ...
    changeDate () {
      return this.$routines.formatDate(this.$props.claimRec.changeDate)
    },
    clTitle () {
