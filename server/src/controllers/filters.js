...
async function saveClaimCondition (req, res, next) {
  try {
    ...
    params.add('P_CLAIM_TYPE').dirIn().typeString().val(filter.claimType)
    params.add('P_CLAIM_STATUS').dirIn().typeString().val(filter.claimStatus)
    params.add('P_CLAIM_VERS').dirIn().typeString().val(filter.claimVersion)
    params.add('P_CLAIM_RELEASE').dirIn().typeString().val(filter.claimRelease)
    params.add('P_CLAIM_BUILD').dirIn().typeString().val(filter.claimBuild)
